	...
	if (row < height && col < width) {		
		...
		for (int filterR = 0; filterR < filterWidth; filterR++) {
			for (int filterC = 0; filterC < filterWidth; filterC++) { 
				...
				int inPixelsR = row - filterWidth / 2 + filterR;
				int inPixelsC = col - filterWidth / 2 + filterC;
				inPixelsR = min(max(0, inPixelsR), height - 1);
				inPixelsC = min(max(0, inPixelsC), width - 1);

				uchar3 inPixel = inPixels[inPixelsR * width + inPixelsC]; 

				x += filterVal * inPixel.x;
				y += filterVal * inPixel.y;
				z += filterVal * inPixel.z;
			}
		}

		int outPixelIndex = row * width + col;
		outPixels[outPixelIndex].x = x;
		outPixels[outPixelIndex].y = y;
		outPixels[outPixelIndex].z = z;
	}
}
        ...
        uchar3 * outPixels)
{
	...
	int smemHeight = blockDim.y + filterWidth -  1;
	int smemWidth = blockDim.x + filterWidth -  1;
	for (int smemRow = threadIdx.y; smemRow < smemHeight; smemRow += blockDim.y) {
		for (int smemCol = threadIdx.x; smemCol < smemWidth; smemCol += blockDim.x)	{
			int inPixelsR = blockIdx.y * blockDim.y - filterWidth / 2 + smemRow;
			int inPixelsC = blockIdx.x * blockDim.x - filterWidth / 2 + smemCol;
			inPixelsR = min(max(0, inPixelsR), height - 1);
			inPixelsC = min(max(0, inPixelsC), width - 1);
	
			s_inPixels[smemRow * smemWidth + smemCol] = inPixels[inPixelsR * width + inPixelsC];
		}
	}
	__syncthreads();
	...
	{		
		...
		for (int filterR = 0; filterR < filterWidth; filterR++)
		{
			for (int filterC = 0; filterC < filterWidth; filterC++)
			{
				...
				int smemRow = threadIdx.y + filterR;
				int smemCol = threadIdx.x + filterC;
				uchar3 s_inPixel = s_inPixels[smemRow * smemWidth + smemCol];

				x += filterVal * s_inPixel.x; 
				y += filterVal * s_inPixel.y;
				z += filterVal * s_inPixel.z;
			}
		}
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		else
		{
			...
			cudaMemcpyToSymbol(dc_filter, filter, filterWidth * filterWidth * sizeof(float));
		}
