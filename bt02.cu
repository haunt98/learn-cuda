...
{
	// TODO
	int i = threadIdx.x * 2 + blockIdx.x * blockDim.x * 2;
...
{
	// TODO
	int preBlocks = blockIdx.x * blockDim.x * 2;
	
	for (int stride = 1; stride < blockDim.x * 2; stride *= 2){
		int cur = preBlocks + threadIdx.x * 2 * stride;
		if (threadIdx.x < blockDim.x / stride){
			if (cur + stride < n){
				in[cur] += in[cur + stride];
			}
		}
		__syncthreads();
	}
...
{
	// TODO
	int cur = blockIdx.x * blockDim.x * 2 + threadIdx.x;
        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		int * d_in, * d_out;
		dim3 gridSize((((n - 1) / 2 + 1) - 1) / blockSize.x + 1); // TODO: Compute gridSize from n and blockSize
